...
curl -i "http://localhost:5080/api/tenant/branding?tenant=clinic1"
curl -i "http://localhost:5173/api/tenant/branding?tenant=clinic1"
```
...
- branding endpoint on backend/proxy returns `200` for `clinic1` (or a handled non-500 if tenant is missing).
